...
/* ------------------------------------------------------------------ */
import { listScanTemplatesToolSchema, listScanTemplatesToolHandler, startScanToolSchema, startScanToolHandler, getScanStatusToolSchema, getScanStatusToolHandler, getScanResultsToolSchema, getScanResultsToolHandler, listScansToolSchema, listScansToolHandler } from './tools/scans.js';
import { getVulnerabilityDetailsToolSchema, getVulnerabilityDetailsToolHandler, searchVulnerabilitiesToolSchema, searchVulnerabilitiesToolHandler } from './tools/vulnerabilities.js';
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
const createServer = () => {
    const server = new Server({ name: 'nessus-server', version: '1.0.0' }, { capabilities: { tools: {} } });
    /* -------- list tools -------- */
...
async function main() {
    try {
        ...
        console.error(`Nessus MCP Server starting in ${apiCfg.useMock ? 'mock' : 'real API'} mode` +
