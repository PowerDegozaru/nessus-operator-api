 ...
 */
import { z } from 'zod';
import { getScanTemplates, startScan, getScanStatus, getScanResults, listScans } from '../nessus-api.js';
import { validateScanId, validateTarget, validateScanType, handleNessusApiError } from '../utils/error-handling.js';
/**
...
export const startScanToolHandler = async (args) => {
    try {
        // Validate arguments
...
export const getScanStatusToolHandler = async (args) => {
    try {
        // Validate arguments
...
export const getScanResultsToolHandler = async (args) => {
    try {
        // Validate arguments
        const scanId = validateScanId(args.scan_id);
        // Get scan results
...
const formatScanResults = (results) => {
    ...
    const sortedVulns = [...results.vulnerabilities].sort((a, b) => {
        return (sortOrder[a.severity] || 999) -
            (sortOrder[b.severity] || 999);
    });
