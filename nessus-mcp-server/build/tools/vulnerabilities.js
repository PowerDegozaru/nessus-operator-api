...
import { validateVulnerabilityId, handleNessusApiError } from '../utils/error-handling.js';
/**
...
export const getVulnerabilityDetailsToolHandler = async (args) => {
    try {
        // Validate arguments
 ...
 */
const formatVulnerabilityDetails = (details) => {
    if (!details) {
    ...
    if (details.affected_systems && details.affected_systems.length > 0) {
        ...
        details.affected_systems.forEach((system) => {
            formatted += `- ${system}\n`;
        });
        formatted += '\n';
    }
...
export const searchVulnerabilitiesToolHandler = async (args) => {
    try {
        // Validate arguments
        ...
        const matches = vulnerabilities.filter(vuln => vuln.name.toLowerCase().includes(keyword) ||
