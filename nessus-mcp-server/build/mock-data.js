...
// Mock scan results
export const generateMockScanResults = (targetIp, scanType) => {
...
export const createMockScan = (target, scanType) => {
    const scanId = `scan-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const scan = {
        id: scanId,
        target: target,
...
export const getMockScanStatus = (scanId) => {
    const scan = mockScans.get(scanId);
    if (!scan) {
...
export const getMockScanResults = (scanId) => {
    const scan = mockScans.get(scanId);
    if (!scan) {
        return { error: "Scan not found" };
    }
...
export const getMockVulnerabilityDetails = (vulnId) => {
